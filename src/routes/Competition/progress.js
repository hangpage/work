        ...
        <div style={{width: 470, paddingTop: 53, margin: '0 auto'}}>
          <Timeline pending={pen} pendingDot={<i/>}>
            ...
            <Timeline.Item dot={<i className='icon-timeline-circle'/>}><p
              className='mb12'>学校内部评审<span>{params.time2}</span>
            </p>{String(teamDetail.schoolReview.status) === '1' ? '晋级下一轮' : '未晋级下一轮'}</Timeline.Item>}
            ...
            </p>{String(teamDetail.firstReview.status) === '1' ? '晋级下一轮' : '未晋级下一轮'}</Timeline.Item>}
            ...
            <Timeline.Item dot={<i className='icon-timeline-circle'/>}><p
              className='mb12'>比赛结束<span>{params.time6}</span>
            </p>{String(teamDetail.endReview.status) === '1' ? '晋级下一轮' : '未晋级下一轮'}</Timeline.Item>}
          </Timeline>
        </div>
      </div>
    </div>
